...
import { reload } from "firebase/auth";
  ...
  app.get("/*", (req, res) => {
    ...
    if (sanitizedPageHeader === "aboutus") {
      console.log("Customizing content for /aboutus");
      htmlcode = htmlcode
        .replace(/src="kai\.png"/g, 'src="https://i.ibb.co/GRRQ9Sv/kai.png')
        .replace(/src="max\.png"/g, 'src="https://i.ibb.co/Yd698VV/max.png')
        .replace(/src="vyom\.png"/g, 'src="https://i.ibb.co/f4rFGHp/vyom.png"');
    }
    else{
      console.log("fail");
    }
    res.send(`
      ...
      <head>
          ...
          <title>Interactive Web ${title || "Content Updater"}</title>
          <style>
      ...
      #siteheader {
          ...
          padding: 15px 30px;
          width: 100%;
          border-bottom: 4px solid #001f44;
    ...
    #content123{
      ...
      /* Container for buttons and textareas */
      #addbut, #dbut2 {
          display: flex;
          flex-wrap: wrap;
  ...
  background: rgba(0,0,0,0.5);
      ...
      window.onerror = function(message, source, lineno, colno, error) {
          ...
          <div id = "content123">
            <address><a href = "/home">Interactive Web Home</a></address>
            <address><a href = "/aboutus">About Us</a></address>
            <address><a href="mailto:maxhzhang119@gmail.com">Reach out</a></address>
            <address><a href = "https://acsty2025.org/aibd/papers">Research AIBD</a></address>          </div>
          <div id="addbut">
          ...
          <footer id="footer">
            <div class="footer-content">
                <p>Interactive Web AIBD 2025</p>
                <p><strong>Kai Zhang, Vyom Kumar, Max Zhang</strong></p>
            </div>
          </footer>
    ...
    if (user) {
      ...
      console.log('No user logged in');
          ...
          <script>
          if (document.title == "Interactive Web aboutus"){
  ...
  function updates(updatetype) {
    ...
    const GPT4Message = [
      {
        role: "system",
        content:"Generate complete HTML, CSS, and JavaScript code with all parts included, separating sections as follows: Use '```html', '```css', and '```javascript', ending each section with '```'. If multiplayer is needed, ALWAYS use the following code: $.ajax({type: \"POST\", url: \"/skibidi\", data: JSON.stringify([put data here]), contentType: \"application/json\", success: response => console.log(\"Confirm and run initiated successfully:\", response), error: error => console.error(\"Error initiating confirm and run:\", error)}); . Provide a '```multiplayer' to handle receiving data, which is inside of the variable 'message' in this section. There is no need for any sort of WebSockets or anything else, just normal data and javascript code. All servers will be provided. Please remember that if you are to send data, you must also make something to receive it. Make sure to call any functions defined in '```multiplayer' inside of itself. HTML should contain only <body> content, using <script type='module'> for libraries. CSS should style all UI for desktop and mobile without images, using only colored <div>s. Our color scheme is from red to purple. Avoid overlaying text on matching background colors. For JavaScript: Include all game logic and send JSON data to the server using AJAX at /skibidi with a data: tag. Use the message variable for communication, Ensure all variables are defined and code is error-free. Fix bugs if line/column numbers are provided. If told to access email, use document.getElementById('useremail').textContent or document.getElementById('username').textContent; both of which are already given. Avoid ${} syntax entirely. If no changes are needed, include the original code,  Otherwise, enhance gameplay mechanics, ensure compatibility with mobile and desktop, and integrate single-player and multiplayer seamlessly. Don't put comments explaining the code. AVOID ANY BODY TAG IN THE CSS. ASSUME BODY CSS IS SET ALREADY",
